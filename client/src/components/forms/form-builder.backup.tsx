                                                ...
                                                onChange={(newConfig) => {
                                                  ...
                                                  setTimeout(() => {
                                                    ...
                                                    // Caso 4: No tiene secciones ni datos iniciales - usar plantilla por defecto
                                                      ...
                                                      // Verificar el caso específico para materias primas 
                                                      // Siempre asumir que son materias primas si no hay secciones pero sí hay datos
                                                      const materiaPrimasDetected = configToSave.initialData && 
                                                          Array.isArray(configToSave.initialData) && 
                                                          configToSave.initialData.length > 0;
                                                      
                                                      if (materiaPrimasDetected) {
                                                        // Estructura especial para materias primas
                                                        console.log("Detectadas materias primas, aplicando estructura adecuada");
                                                        configToSave = {
                                                          rows: configToSave.initialData?.length || 13,
                                                          dynamicRows: false,
                                                          sections: [
                                                            {
                                                              title: "Materia Prima",
                                                              columns: [
                                                                { 
                                                                  id: Object.keys(configToSave.initialData[0])[0] || "materia_prima",
                                                    ...
                                                    try {
                                                      setTimeout(() => {
                                                        try {
                                                          ...
                                                          console.error("FormBuilder: Error dentro del timeout", innerError);
                                                          form.setValue(`fields.${index}.advancedTableConfig`, newConfig);
                                                        }
